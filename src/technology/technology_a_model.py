...
def production(scale, capital, lifetime, fixed, input, parameter):
  ...
  return np.stack((scale * parameter[10] * parameter[11] * parameter[12] * input[0],
                   scale * parameter[13] * input[1],
                   scale * parameter[13] * input[0]))
...
def metrics(scale, capital, lifetime, fixed, input_raw, input, input_price, output_raw, output, cost, parameter):
  ...
  parameter : array
    The technological parameterization.
  """
  _annualized_cost_per_unit_F = (capital[0] + capital[1] + capital[2] + capital[3] + capital[4] + fixed*lifetime[0] + input_price[0] * input_raw[0] + input_price[1] * input_raw[1] + input_price[2] * input_raw[2])/output[0]
  ...
  _overall_efficiency = output[0] / (input_raw[0] + input_raw[1] + input_raw[2])

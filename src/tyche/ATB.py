...
class ATB:
    ...
    def __init__(
        ...
        if not os.path.isfile(os.path.join(path, self.tech_filename)):
          print(f"Investments: No input data found in {os.path.join(path, self.atb_tech)}")
          sys.exit(1)
        else:
            self.atb_data = pd.read_csv(os.path.join(path, self.tech_filename))
    ...
    def process_data(self, parmaeters):
        pass
